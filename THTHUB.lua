...
local ToggleButton = Instance.new("ImageButton") -- Đổi từ TextButton sang ImageButton
...
ToggleButton.Image = "http://www.roblox.com/asset/?id=121516369236727" -- ICON CUSTOM
...
-- Hàm tạo toggle
local function createToggle(tabData, text, description, default, callback)
    ...
    titleLabel.BackgroundTransparency = 1
    titleLabel.Position = UDim2.new(0, 15, 0, 5)
    titleLabel.Size = UDim2.new(0.7, -15, 0, 20)
    titleLabel.Text = text
    titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
    titleLabel.TextSize = 14
    titleLabel.Font = Enum.Font.GothamBold
    ...
    descLabel.BackgroundTransparency = 1
    descLabel.Position = UDim2.new(0, 15, 0, 25)
    descLabel.Size = UDim2.new(0.7, -15, 0, 20)
    descLabel.Text = description
    descLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
    descLabel.TextSize = 12
    descLabel.Font = Enum.Font.Gotham
...
createButton(nightTab, "Combo New", "No Key - Ổn đỉnh", function()
...
createToggle(settingsTab, "Fix Lag Siêu Cấp", "Xoá map và tối ưu đồ hoạ", false, function(state)
    ...
    if state then
        ...
        for _,v in pairs(workspace:GetChildren()) do
            if not (v == LocalPlayer.Character or v.Name == "Camera") then
...
-- QUAN TRỌNG: Nút toggle UI với icon custom
ToggleButton.MouseButton1Click:Connect(function()
...
print("✅ Nhấn icon custom để bật/tắt UI")
print("✅ Kéo icon để di chuyển")
print("✅ Kéo title bar để di chuyển UI chính")